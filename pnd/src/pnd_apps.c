...
unsigned char pnd_apps_exec(char *pndrun, char *fullpath, char *unique_id,
                            ...
                            unsigned int clockspeed __attribute__((unused)), unsigned int options)
{
    char *argv[20];
    int f;
...
void pnd_get_ro_mountpoint(char *fullpath __attribute__((unused)), char *unique_id, char *r_mountpoint, unsigned int mountpoint_len)
{
    if (!r_mountpoint)
        return; // sillyness

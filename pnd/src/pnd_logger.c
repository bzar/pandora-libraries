...
struct pnd_log_target_t
{
    enum pnd_logger_e type;
    union
    {
        FILE *stream;
...
static struct pnd_log_target_t log_targets[PND_LOG_MAX]; // implicitly nil
...
{
    memset(log_targets, '\0', sizeof(struct pnd_log_target_t)* PND_LOG_MAX);
    return;
}
...
unsigned char pnd_log_to_syslog(char *facility __attribute__((unused)))
{
    return 0; // NYI
}

unsigned char pnd_log_to_callback(pnd_log_callback_f f __attribute__((unused)), void *userdata __attribute__((unused)))
{
    return 0; // NYI
}

...
#define PND_DOTDESKTOP_SOURCE "X-Pandora-Source=libpnd"
    unsigned char pnd_emit_dotdesktop(char *targetpath, char *pndrun, struct pnd_disco_t *p);
...
// to the given directory; returns 1 on sucess, otherwise is a fail.
    unsigned char pnd_emit_icon(char *targetpath, struct pnd_disco_t *p);
    unsigned char *pnd_emit_icon_to_buffer(struct pnd_disco_t *p, unsigned int *r_buflen); // returns length of malloc in r_buflen if !NULL
...
//   scenario, but it currently is not coded like that.
    int pnd_map_dotdesktop_categories(pnd_conf_handle c, char *target_buffer, unsigned short int len, struct pnd_disco_t *d); // <---

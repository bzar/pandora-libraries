...
struct generic_event_t generics[] =
{
...
struct evmap_t g_evmap[MAXEVENTS];
    ...
    */
    ...
    while (n)
    {
        ...
        else if (strncmp(k, "events.", 7) == 0)
        {
            ...
            // figure out which keycode we're talking about
            struct generic_event_t *p = generics;
...
void dispatch_key(int keycode, int val)
{
    ...
    for (i = 0; i < g_evmap_max; i++)
    {
        ...
        if ((g_evmap[i].key_p) &&
                (((struct keycode_t*)(g_evmap[i].reqs))-> keycode == keycode) &&
                (g_evmap[i].script))
        {
...
void dispatch_event(int code, int val)
{
    ...
    for (i = 0; i < g_evmap_max; i++)
    {
        ...
        if ((g_evmap[i].key_p == 0) &&
                (((struct generic_event_t*)(g_evmap[i].reqs))-> code == code) &&
                (g_evmap[i].script))
        {
